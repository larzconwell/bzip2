...
func NewWriter(w io.Writer) *Writer {
	return &Writer{bw: newBitWriter(w), blockSize: 6 * baseBlockSize}
}

// NewWriterLevel is like NewWriter except a specific blockSize is given to
// control the level of compression.
//
// The blockSize must be between 1 and 9, any other values are set to the closed
// valid blockSize.
func NewWriterLevel(w io.Writer, blockSize int) *Writer {
	return &Writer{bw: newBitWriter(w), blockSize: blockSize * baseBlockSize}
}
...
func (b *Writer) writeHeader() error {
	...
	b.bw.WriteBits(8, 'h')
	b.bw.WriteBits(8, uint64('0'+b.blockSize/baseBlockSize))
